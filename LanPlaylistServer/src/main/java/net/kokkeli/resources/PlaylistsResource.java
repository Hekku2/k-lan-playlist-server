...
public class PlaylistsResource extends BaseResource {
    ...
    protected PlaylistsResource(ILogger logger,
            ...
            IPlaylistService playlistService, IFileSystem filesystem) {
    ...
    public Response add(@Context HttpServletRequest req,
        ...
        try {
            if (ValidationUtils.isEmpty(track)
                    || ValidationUtils.isEmpty(artist)) {
            ...
            if (!ValidationUtils
                    .containsOnlyNumbersAndLettersAndWhiteSpace(track)
                    || !ValidationUtils
                            .containsOnlyNumbersAndLettersAndWhiteSpace(artist)) {
            ...
            log("User trying to upload file: " + fileDetail.getFileName()
                    + ", Filetype: " + fileDetail.getType(), LogSeverity.TRACE);
            String filename = settings.getTracksFolder() + "/"
                    + fileDetail.getFileName();

            if (filesystem.fileExists(filename)) {
                model.setError("Similar file already exists. Remove existing file, or upload different.");
                log("User tried to upload file with same name with file already in system. File: "
                        + filename, LogSeverity.TRACE);
                return Response.ok(
                        templates.process(PLAYLIST_TRACK_ADD_TEMPLATE, model))
                        .build();
            }
            ...
            log("There was a problem with IO:" + e.getMessage(),
                    LogSeverity.ERROR);
            throw new ServiceException(
                    "There was a problem with file uploading.", e);
        }
    }
    ...
    @Access(Role.USER)
    @Path("/{playlistId: [0-9]*}")
    public Response details(@Context HttpServletRequest req,
            @PathParam("playlistId") long playlistId) throws ServiceException,
            NotAuthenticatedException {
        ...
        try {
            ...
            sessions.setError(baseModel.getCurrentSession().getAuthId(),
                    "Playlist not found.");
    ...
    public Response create(@Context HttpServletRequest req)
        ...
        try {
            return Response.ok(
                    templates.process(PLAYLIST_CREATE_TEMPLATE, baseModel))
                    .build();
        } catch (RenderException e) {
            return handleRenderingError(baseModel, e);
        }
            ...
            if (playlistService.nameExists(item.getName())) {
                baseModel
                        .setError(String.format(
                                "Playlist with name %s already exits.",
                                item.getName()));
                baseModel.setModel(item);
                return Response.ok(
                        templates.process(PLAYLIST_CREATE_TEMPLATE, baseModel))
     ...
     */
    private ModelPlaylist createPlaylist(
            MultivaluedMap<String, String> formParams)
            throws BadRequestException {
