...
(cd k-lan-playlist-server/LanPlayerClientSide/ && npm install)
...
su - vagrant -c 'cd k-lan-playlist-server/UserService/ && pm2 -s start src/start.js --watch --name user-service'pm2
